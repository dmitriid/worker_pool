...
start_link(Options) when is_list(Options) ->
    ...
    ServerName = list_to_atom(atom_to_list(Server) ++ "_sup"),
...
init(Options) ->
    WCount = proplists:get_value(worker_count, Options, erlang:system_info(schedulers)),
    {ok, {{one_for_one, 10, 10},
          [{worker_pool, {worker_pool, start_link, [Options
                                                    ]}, transient,
            16#ffffffff, worker, [worker_pool]} |
           [{N, {worker_pool_worker, start_link, [Options ++ [{worker_id, N}]]
                }, transient, 16#ffffffff,
